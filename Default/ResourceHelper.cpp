...
UINT GetString(IMAGE_RESOURCE_DATA_ENTRY* pdata, UINT uid, LPCTSTR* ppstr)
{
LPCTSTR pblock=(LPCTSTR)&pBase[pdata->OffsetToData];
UINT ublocksize=pdata->Size/sizeof(TCHAR);
UINT ustr=0;
for(UINT ublockpos=0; ublockpos<ublocksize; )
	{
	TCHAR ulen=pblock[ublockpos];
	ublockpos++;
	if(ustr==uid)
		{
...
UINT GetResourceString(UINT uid, LPCTSTR* ppstr)
{
ASSERT(uid&&ppstr);
...
try
	...
	ulen=GetString(pdata, ublockid, &ptmp);
	}
catch(UINT uerr)
	{
	if(uerr!=EINVAL)
...
UINT GetResourceString(UINT uid, LPTSTR pstr, UINT umaxlen)
{
ASSERT(uid&&pstr&&umaxlen);
LPCTSTR pres=nullptr;
UINT ulen=GetResourceString(uid, &pres);
if(ulen>umaxlen-1)
	throw ENOMEM;
StringCopy(pstr, umaxlen, pres);
pstr[ulen]=0;
return ulen;
}
