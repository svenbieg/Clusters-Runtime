...
public:
	...
	virtual _item_t const& get_at(size_t position)const=0;
	...
	// Modification
	virtual bool append(_item_t const& item, bool again)=0;
	virtual size_t append(_item_t const* append, size_t count)=0;
	virtual bool insert_at(size_t position, _item_t const& item, bool again)=0;
...
public:
	...
	inline _item_t const& get_at(size_t position)const override
		{
		if(position>=_m_item_count)
			throw std::invalid_argument("");
		return get_items()[position];
	...
	// Modification
	bool append(_item_t const& item, bool)override
		{
	...
	bool insert_at(size_t position, _item_t const& item, bool)override
		{
		if(position>_m_item_count)
			throw std::invalid_argument("");
		if(position==_m_item_count)
...
public:
	...
	_item_t const& get_at(size_t position)const override
		{
		unsigned int group=get_group(&position);
		if(group>=_group_size)
			throw std::invalid_argument("");
	...
	// Modification
	bool append(_item_t const& item, bool again)override
		{
	...
	bool insert_at(size_t position, _item_t const& item, bool again)
		{
		if(position>_m_item_count)
			throw std::invalid_argument("");
		size_t pos=position;
...
public:
	...
	inline void insert_at(size_t position, _item_t const& item)
		{
...
protected:
	// Using
	using _base=_list_iterator_base<_item_t, _group_size, _is_const>;
	using _group=_list_group<_item_t>;
	using _item_group=_list_item_group<_item_t, _group_size>;
	using _list=_list_cluster<_item_t, _group_size>;
	using _list_ptr=typename std::conditional<_is_const, _list const*, _list*>::type;
	using _parent_group=_list_parent_group<_item_t, _group_size>;
...
private:
	// Using
	using _base=_list_iterator_base<_item_t, _group_size, false>;
...
private:
	// Using
	using _base=_list_iterator_base<_item_t, _group_size, true>;
	using _it=_list_const_iterator<_item_t, _group_size>;
	using _list=_list_cluster<_item_t, _group_size>;
